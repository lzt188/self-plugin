...
4. **增量缓存层** `collectUsage()` —— live/persisted 双来源、revision 游标、原子落盘 `usage-cache.json`。
...
- 服务端 `CACHE_VERSION` 升级（当前 = v3）会令 `usage-cache.json` 整体失效重算；Claude cache 版本独立。
